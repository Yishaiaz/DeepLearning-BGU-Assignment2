...
class TestSiameseNetworkPreProcessing(TestCase):
    ...
    def test_get_single_image(self):
        ...
        true_im = cv2.imread(full_path_to_image, cv2.IMREAD_GRAYSCALE)
        # test with no flattening
        np.testing.assert_array_almost_equal(get_single_image(name=name,
                                                              image_idx=im_idx,
                                                              normalize_im=False,
                                                              flatten_im=False), true_im)
        # test with flattening
        np.testing.assert_array_almost_equal(get_single_image(name=name,
                                                              image_idx=im_idx,
                                                              normalize_im=False,
                                                              flatten_im=True), true_im.flatten())

        # test with normalization
        np.testing.assert_array_almost_equal(get_single_image(name=name,
                                                              image_idx=im_idx,
                                                              flatten_im=False,
                                                              normalize_im=True), true_im/255)
        # test with normalization and flattening
        np.testing.assert_array_almost_equal(get_single_image(name=name,
                                                              image_idx=im_idx,
                                                              flatten_im=True,
                                                              normalize_im=True), true_im.flatten()/255)

    def test_load_images_as_vectors(self):
        matching_df = pd.DataFrame({"name": ['Aaron_Peirsol'], 'n1': [1], 'n2': [2]})
        non_matching_df = pd.DataFrame()

        org_image1 = cv2.imread('lfw2Data/lfw2/Aaron_Peirsol/Aaron_Peirsol_0001.jpg', cv2.IMREAD_GRAYSCALE)/255
        org_image2 = cv2.imread('lfw2Data/lfw2/Aaron_Peirsol/Aaron_Peirsol_0002.jpg', cv2.IMREAD_GRAYSCALE)/255

        image_pairs, pairs_labels, pairs_names = load_images_as_vectors(matching_df, non_matching_df)
        im1_p1 = image_pairs[0][0]
        im2_p1 = image_pairs[0][1]
        label = pairs_labels[0]
        name = pairs_names[0][0]
        self.assertEqual(1, label)
        self.assertEqual('Aaron_Peirsol', name)
        ...
        org_image1 = cv2.imread('lfw2Data/lfw2/Aaron_Peirsol/Aaron_Peirsol_0003.jpg', cv2.IMREAD_GRAYSCALE)/255
        org_image2 = cv2.imread('lfw2Data/lfw2/Abdoulaye_Wade/Abdoulaye_Wade_0001.jpg', cv2.IMREAD_GRAYSCALE)/255

        image_pairs, pairs_labels, pairs_names = load_images_as_vectors(matching_df, non_matching_df)
        im1_p1 = image_pairs[0][0]
        im1_p2 = image_pairs[0][1]
        label = pairs_labels[0]
        name1 = pairs_names[0][0]
        name2 = pairs_names[0][1]
        self.assertEqual(0, label)
        self.assertEqual('Aaron_Peirsol', name1)
        self.assertEqual('Abdoulaye_Wade', name2)
        np.testing.assert_array_almost_equal(im1_p1, org_image1)
    ...
    def test_train_val_split(self):
        ...
        image_pairs, pairs_labels, pairs_names = load_images_as_vectors(matching_df, non_matching_df)
        train_dataset, train_batched_dataset, names_in_train, val_dataset, val_batched_dataset, names_in_val = \
            train_val_split(image_pairs, pairs_labels, pairs_names, shuffle=False, val_percent=.5)
